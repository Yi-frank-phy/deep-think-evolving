...
- **最终目标**:
  通过**模拟退火（Simulated Annealing）**思想，我们让“温度 T”不再按固定日程衰减，而是随种群熵的变化自然调节：熵高时温度高，驱动更大的 UCB 探索项；熵低时温度低，使策略聚焦于利用。由此，剪枝策略能够在无人工设定的降温曲线下，从前期的广度探索平滑过渡到后期的深度利用，最终稳定在高质量的解上。
...
根据您的最新指示，我们将放弃基于固定深度的温度衰减策略，转而采用一种更智能的、基于种群熵驱动的自适应温度控制机制：温度 `T` 由熵实时计算，并作为 UCB（Upper Confidence Bound）评分中的探索项输入。其核心是：**系统的“温度”应当直接反映其当前“不确定性”或“探索水平”**。

在我们的系统中，**种群的概念熵 `H(Concepts)`** 正是衡量系统多样性和不确定性的完美指标。

剪枝阶段的整体打分可形式化为 `UCB(c) = Value_Score(c) + T * Exploration_Bonus(c)`：`Value_Score` 负责利用项，而 `Exploration_Bonus`（基于统计置信区间）由温度 `T` 调节强度，确保熵越高的时刻探索系数越大。
