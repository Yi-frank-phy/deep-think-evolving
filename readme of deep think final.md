...
3.2.4. 【新增】高级探索机制：能级-对数概率图与线性化温度 (Energy-LogProbability Plot & Linearized Temperature)

**3.2.4.1. 理论背景：从超验方程到线性回归**
之前的尝试试图通过宏观统计量“熵 ($S$)”去反推微观参数“温度 ($T$)”，这导致了复杂的积分和超越方程。回归**玻尔兹曼分布的代数定义本身**，我们发现这是一个简单的**线性关系**。

玻尔兹曼分布定义为 $p_i = \frac{1}{Z} e^{V_i / T}$。对两边取自然对数：
$$
\ln p_i = \frac{V_i}{T} - \ln Z
$$
这构成了一个标准的线性方程 $y = kx + b$，其中 $y = \ln p_i$，$x = V_i$，斜率 $k = 1/T$。
因此，**“对数概率 ($\ln p$)”与“价值 ($V$)”之间呈严格的线性关系**。

**3.2.4.2. 有效温度 ($T_{eff}$) 的闭式解**
我们不再需要迭代求解，而是利用最小二乘法 (Least Squares) 直接估计当前种群分布的“等效温度”。
温度 $T_{eff}$ 的闭式解为斜率 $k$ 的倒数：
$$
T_{eff} = \left| \frac{\text{Var}(V)}{\text{Cov}(V, \ln \hat{p})} \right| = \left| \frac{\sum_{i=1}^N (V_i - \bar{V})^2}{\sum_{i=1}^N (V_i - \bar{V}) (\ln \hat{p}(v_i) - \overline{\ln p})} \right|
$$
*(注：取绝对值是为了确保探索驱动力为正，即使在高价值区样本稀疏的反常情况下。)*

**物理意义**：该公式计算的是**“当前这群粒子的分布形态，最像是在多少度温度下的热平衡态？”**
* 如果分布平坦（$\ln p$ 变化小），则 $T_{eff} \to \infty$（高温）。
* 如果分布集中于高价值区（$\ln p$ 随 $V$ 剧增），则 $T_{eff} \to 0$（低温）。

**3.2.4.3. 归一化温度与 UCB 评分**
定义**归一化温度 (Normalized Temperature, $\tau$)** 为有效采样温度与系统最高允许温度（超参数）的比值：
$$
\tau = \frac{T_{eff}}{T_{max}}
$$
$\tau$ 实际上衡量了当前系统的“自然探索程度”。我们将 $\tau$ 代入 UCB 公式：
$$
\text{Score}(x) = \frac{V(x) - V_{\min}}{V_{\max} - V_{\min} + \epsilon} + c \cdot \tau \cdot \frac{1}{\sqrt{\hat{p}(v(x))}}
$$
该机制利用初等函数闭式解 ($O(N)$)，无需迭代，数值极其稳健，完美实现了从观测分布到探索控制的闭环。
